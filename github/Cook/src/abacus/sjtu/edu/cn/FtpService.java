...
public class FtpService {
	...
	public static FTPClient createFtpClient(FtpInfo ftpInfo)
		...
		int timeout = Integer.parseInt(System.getProperty("config.ftpTimeout",
				"60")) * 1000;
		...
		if (FTPReply.isPositiveCompletion(ftpClient.getReplyCode())) {
			...
			throw new RuntimeException("Failed to connect the FTP server: "
					+ ftpInfo.getHost());
		}
		...
		if (ftpClient.login(ftpInfo.getUserId(), password)) {
			...
			System.err.println(ftpClient.getReplyString());
			System.err.println("FTP HOST:" + ftpInfo.getHost()
					+ " FTP User ID:" + ftpInfo.getUserId() + " FTP Port:"
					+ ftpInfo.getPort());
	...
	private static void connect(FtpInfo ftpInfo, FTPClient ftpClient)
			throws IOException {
		System.out.println("try to connect FTP. Host : " + ftpInfo.getHost()
				+ ", port : " + ftpInfo.getPort());
		long retryInterval = Long.parseLong(System.getProperty(
				"config.retryInterval", "10000"));
		int retryCount = Integer.parseInt(System.getProperty(
				"config.retryCount", "5"));
		boolean retry = false;
		do {
			...
			if (retry) {
				System.out.println("Will retry to connect ftp server "
						+ ftpInfo.getHost() + ". Retry count left: "
						+ retryCount);
				try {
	...
	public static void cleanExpirationFile(FTPClient ftpClient,
			String remoteDir, Date expirationDate) throws IOException {
		removeDirectory(ftpClient, remoteDir, expirationDate, false);
	}

	public static void removeDirectory(FTPClient ftpClient, String remoteDir)
			throws IOException {
		removeDirectory(ftpClient, remoteDir, null, true);
	}

	private static void removeDirectory(FTPClient ftpClient, String remoteDir,
			Date expirationDate, boolean delRootDir) throws IOException {
	...
	private static void logDeleteResult(String path, boolean isDir,
	...
	private static boolean needDelete(FTPFile file, Date expirationDate) {
		return expirationDate == null
				|| file.getTimestamp().getTime().before(expirationDate);
	}
